...
static void CB2_LoadRoulette(void)
{
    ...
    case 6:
        ...
        AddTextPrinterParameterized(sTextWindowId, 2, Roulette_Text_ControlsInstruction, 0, 1, TEXT_SPEED_FF, NULL);
...
static void Task_AskKeepPlaying(u8 taskId)
{
    ...
    AddTextPrinterParameterized(sTextWindowId, 2, Roulette_Text_KeepPlaying, 0, 1, TEXT_SPEED_FF, 0);
    CopyWindowToVram(sTextWindowId, 3);
...
static void Task_PrintSpinResult(u8 taskId)
{
    ...
    case 2: // never happens
        if (gTasks[taskId].tMultiplier == MAX_MULTIPLIER)
        {
            ...
            DrawStdWindowFrame(sTextWindowId, FALSE);
            AddTextPrinterParameterized(sTextWindowId, 2, Roulette_Text_Jackpot, 0, 1, TEXT_SPEED_FF, NULL);
            CopyWindowToVram(sTextWindowId, 3);
        }
        else
        {
            ...
            DrawStdWindowFrame(sTextWindowId, FALSE);
            AddTextPrinterParameterized(sTextWindowId, 2, Roulette_Text_ItsAHit, 0, 1, TEXT_SPEED_FF, NULL);
            CopyWindowToVram(sTextWindowId, 3);
        }
        break;
    case FALSE:
    default:
        ...
        DrawStdWindowFrame(sTextWindowId, FALSE);
        AddTextPrinterParameterized(sTextWindowId, 2, Roulette_Text_NothingDoing, 0, 1, TEXT_SPEED_FF, NULL);
        CopyWindowToVram(sTextWindowId, 3);
        break;
    }
    gTasks[taskId].data[1] = 0;
...
static void Task_PrintPayout(u8 taskId)
{
    ...
    DrawStdWindowFrame(sTextWindowId, FALSE);
    AddTextPrinterParameterized(sTextWindowId, 2, gStringVar4, 0, 1, TEXT_SPEED_FF, NULL);
    CopyWindowToVram(sTextWindowId, 3);
...
static void Task_TryPrintEndTurnMsg(u8 taskId)
{
    ...
    if (gTasks[taskId].tCoins >= sRoulette->minBet)
    {
        if (gTasks[taskId].tBallNum == BALLS_PER_ROUND)
        {
            ...
            DrawStdWindowFrame(sTextWindowId, FALSE);
            AddTextPrinterParameterized(sTextWindowId, 2, Roulette_Text_BoardWillBeCleared, 0, 1, TEXT_SPEED_FF, NULL);
            CopyWindowToVram(sTextWindowId, 3);
        ...
        else if (gTasks[taskId].tCoins == MAX_COINS)
        {
            ...
            DrawStdWindowFrame(sTextWindowId, FALSE);
            AddTextPrinterParameterized(sTextWindowId, 2, Roulette_Text_CoinCaseIsFull, 0, 1, TEXT_SPEED_FF, NULL);
            CopyWindowToVram(sTextWindowId, 3);
    ...
    else
    {
        // Player out of coins
        DrawStdWindowFrame(sTextWindowId, FALSE);
        AddTextPrinterParameterized(sTextWindowId, 2, Roulette_Text_NoCoinsLeft, 0, 1, TEXT_SPEED_FF, NULL);
        CopyWindowToVram(sTextWindowId, 3);
...
static void Task_PrintMinBet(u8 taskId)
{
    if (JOY_NEW(A_BUTTON | B_BUTTON))
    {
        ...
        AddTextPrinterParameterized(0, 2, gStringVar4, 0, 1, TEXT_SPEED_FF, NULL);
...
static void Task_PrintRouletteEntryMsg(u8 taskId)
{
    ...
    if (gTasks[taskId].tCoins >= minBet)
    {
        if ((gSpecialVar_0x8004 & ROULETTE_SPECIAL_RATE) && (gSpecialVar_0x8004 & 1))
        {
            ...
            DrawStdWindowFrame(0, FALSE);
            AddTextPrinterParameterized(0, 2, Roulette_Text_SpecialRateTable, 0, 1, TEXT_SPEED_FF, NULL);
            CopyWindowToVram(0, 3);
