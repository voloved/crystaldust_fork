...
static void InitPartyMenu(u8 menuType, u8 layout, u8 partyAction, bool8 keepCursorPos, u8 messageId, TaskFunc task, MainCallback callback)
{
    ...
    else
    {
        ...
        if (menuType == PARTY_MENU_TYPE_CHOOSE_HALF && GetMaxBattleEntries() > 1)
            sPartyMenuInternal->showConfirmButton = TRUE;
        else
            sPartyMenuInternal->showConfirmButton = FALSE;
...
static bool8 ShowPartyMenu(void)
{
    ...
    case 17:
        ...
        break;
    case 18:
        CreateCancelConfirmWindows(sPartyMenuInternal->showConfirmButton);
        gMain.state++;
        break;
    case 19:
        gMain.state++;
...
static void CreateCancelConfirmPokeballSprites(void)
{
    ...
    {
        if (sPartyMenuInternal->showConfirmButton)
        {
...
static void CreateCancelConfirmWindows(bool8 showConfirmButton)
{
    u8 confirmWindowId;
    u8 cancelWindowId;
    u8 offset;
...
static void DisplayPartyPokemonDescriptionText(u8 stringID, struct PartyMenuBox *menuBox, u8 c)
{
    ...
    if (c != 2)
        AddTextPrinterParameterized3(menuBox->windowId, 2, menuBox->infoRects->descTextLeft, menuBox->infoRects->descTextTop, sFontColorTable[0], 0, sDescriptionStringTable[stringID]);
}
...
static void PartyMenuPrintText(const u8 *text)
{
    ...
    AddTextPrinterParameterized2(6, 2, text, GetPlayerTextSpeedDelay(), 0, 2, 1, 3);
}
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        for (j = 0; sFieldMoves[j] != FIELD_MOVE_TERMINATOR; j++)
        {
            ...
            && sPartyMenuInternal->numActions < (5 - displayRelearn)) //Save one slot for the relearn option if it's on
            {
...
static void CursorCb_Relearn(u8 taskId)
{
    ...
    FlagSet(FLAG_PARTY_MOVES);  // Return to Party menu
...
void ItemUseCB_PokeBall(u8 taskId, TaskFunc task)
{
    ...
    static const u8 sText_MonBallWasChanged[] = _("{STR_VAR_1} was swapped to a {STR_VAR_2}.\nThe {STR_VAR_3} was put back in the BAG.{PAUSE_UNTIL_PRESS}");
...
static void Task_ItemUseCB_PokeBallBreakYesNoInput(u8 taskId)
{
    ...
    u16 currBall = GetMonData(mon, MON_DATA_POKEBALL);
    u16 newBall = gSpecialVar_ItemId;
    static const u8 sText_MonBallWasChangedBallBroke[] = _("{STR_VAR_1} was swapped to a {STR_VAR_2}.\nThe {STR_VAR_3} broke when removing it.{PAUSE_UNTIL_PRESS}");
