...
{   // { width, height, offset }
    ...
    [MENU_INFO_ICON_PSS_STAT]  = { 12, 12, 0x0E },
};
...
void WindowFunc_DrawDialogueFrame(u8 bg, u8 tilemapLeft, u8 tilemapTop, u8 width, u8 height, u8 paletteNum)
{
    ...
    FillBgTilemapBufferRect(bg,
                            ...
                            DLG_WINDOW_PALETTE_NUM);
    FillBgTilemapBufferRect(bg,
                            BG_TILE_V_FLIP(DLG_WINDOW_BASE_TILE_NUM + 10),
                            tilemapLeft - 2,
                            tilemapTop + 2,
                            1,
                            1,
                            DLG_WINDOW_PALETTE_NUM);
    FillBgTilemapBufferRect(bg,
                            BG_TILE_V_FLIP(DLG_WINDOW_BASE_TILE_NUM + 11),
                            tilemapLeft - 1,
                            tilemapTop + 2,
                            1,
                            1,
                            DLG_WINDOW_PALETTE_NUM);
    FillBgTilemapBufferRect(bg,
                            BG_TILE_V_FLIP(DLG_WINDOW_BASE_TILE_NUM + 12),
                            tilemapLeft + width,
                            tilemapTop + 2,
                            1,
                            1,
                            DLG_WINDOW_PALETTE_NUM);
    FillBgTilemapBufferRect(bg,
                            BG_TILE_V_FLIP(DLG_WINDOW_BASE_TILE_NUM + 13),
                            tilemapLeft + width + 1,
                            tilemapTop + 2,
                            1,
                            1,
    ...
    FillBgTilemapBufferRect(bg,
                            ...
                            DLG_WINDOW_PALETTE_NUM);
    FillBgTilemapBufferRect(bg,
                            BG_TILE_V_FLIP(DLG_WINDOW_BASE_TILE_NUM + 8),
                            tilemapLeft + width,
                            tilemapTop + 3,
                            1,
                            1,
                            DLG_WINDOW_PALETTE_NUM);
    FillBgTilemapBufferRect(bg,
                            BG_TILE_V_FLIP(DLG_WINDOW_BASE_TILE_NUM + 9),
                            tilemapLeft + width + 1,
                            tilemapTop + 3,
                            1,
                            1,
...
u8 AddStartMenuWindow(u8 numActions)
{
    if (sStartMenuWindowId == WINDOW_NONE)
        sStartMenuWindowId = AddWindowParameterized(0, 22, 1, 7, (numActions * 2) - 1, 15, 0x13D);
...
void PrintMenuTable(u32 windowId, u8 itemCount, const struct MenuAction *menuActions)
{
    ...
    for (i = 0; i < itemCount; i++)
    {
        AddTextPrinterParameterized(windowId, 2, menuActions[i].text, 8, (i * 14) + 1, 0xFF, NULL);
    }
...
void PrintMenuGridTable(u32 windowId, u8 optionWidth, u8 columns, u8 rows, const struct MenuAction *menuActions)
{
    ...
    {
        for (j = 0; j < columns; j++)
            AddTextPrinterParameterized(windowId, 2, menuActions[(i * columns) + j].text, (optionWidth * j) + 8, (i * 16) + 1, 0xFF, NULL);
    }
    CopyWindowToVram(windowId, 2);
}
    ...
    switch (textId)
    {
        ...
        case SAVE_MENU_CAUGHT:
            if (IsNationalPokedexEnabled())
                ...
                string = ConvertIntToDecimalStringN(string, GetJohtoPokedexCount(FLAG_GET_CAUGHT), STR_CONV_MODE_LEFT_ALIGN, 3);
            *string = EOS;
            break;
