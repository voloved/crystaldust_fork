...
static void PrintTitle(void)
{
    ...
    PrintEasyChatTextWithColors(0, 2, titleText, xOffset, 1, TEXT_SPEED_FF, TEXT_COLOR_TRANSPARENT, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_LIGHT_GRAY);
    PutWindowTilemap(0);
...
static void PrintEasyChatStdMessage(u8 msgId)
{
    ...
    if (text1)
        PrintEasyChatText(1, 2, text1, 0, 1, TEXT_SPEED_FF, 0);

    if (text2)
        PrintEasyChatText(1, 2, text2, 0, 17, TEXT_SPEED_FF, 0);
...
static void PrintCurrentPhrase(void)
{
    ...
    for (i = 0; i < numRows; i++)
    {
        ...
        *str = EOS;
        PrintEasyChatText(sScreenControl->windowId, 2, sScreenControl->phrasePrintBuffer, 0, i * 16 + 1, TEXT_SPEED_FF, 0);
    }
...
static void PrintKeyboardGroupNames(void)
{
    ...
    while (1)
    {
        for (x = 0; x < 2; x++)
        {
            ...
            PrintEasyChatText(2, 2, GetEasyChatWordGroupName(groupId), x * 84 + 10, y, TEXT_SPEED_FF, NULL);
        }
...
static void PrintKeyboardAlphabet(void)
{
    ...
    for (i = 0; i < ARRAY_COUNT(sEasyChatKeyboardAlphabet); i++)
        PrintEasyChatText(2, 2, sEasyChatKeyboardAlphabet[i], 10, 97 + i * 16, TEXT_SPEED_FF, NULL);
}
    ...
    for (i = 0; i < numRows; i++)
    {
        for (j = 0; j < 2; j++)
        {
            ...
            if (easyChatWord != EC_EMPTY_WORD)
            {
                ...
                if (!DummyWordCheck(easyChatWord))
                    PrintEasyChatText(2, 2, sScreenControl->wordSelectPrintBuffer, (j * 13 + 3) * 8, y, TEXT_SPEED_FF, NULL);
                else // Never reached
                    PrintEasyChatTextWithColors(2, 2, sScreenControl->wordSelectPrintBuffer, (j * 13 + 3) * 8, y, TEXT_SPEED_FF, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_RED, TEXT_COLOR_LIGHT_GRAY);
            }
        }
...
static void TryAddInterviewObjectEvents(void)
{
    ...
    case EASY_CHAT_PERSON_REPORTER_MALE:
        graphicsId = OBJ_EVENT_GFX_ZREPORTER_M;
        break;
    case EASY_CHAT_PERSON_REPORTER_FEMALE:
        graphicsId = OBJ_EVENT_GFX_ZREPORTER_F;
        break;
...
static void AddMainScreenButtonWindow(void)
{
    ...
    for (i = 0; i < (int)ARRAY_COUNT(sFooterTextOptions[0]); i++)
    {
        ...
        if (str)
        {
            ...
            PrintEasyChatText(windowId, 2, str, x, 1, 0, NULL);
        }
    }
