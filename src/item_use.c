...
static void DisplayOaksAdviceCannotUseItemMessage(u8 taskId, bool8 isUsingRegisteredKeyItemOnField)
{
    DisplayCannotUseItemMessage(taskId, isUsingRegisteredKeyItemOnField, gText_OaksAdvice);
}
...
void ItemUseOutOfBattle_TMHM(u8 taskId)
{
    if (gSpecialVar_ItemId >= ITEM_HM01_CUT)
        DisplayItemMessage(taskId, 2, gText_BootedUpHM, BootUpSoundTMHM); // HM
    else
        DisplayItemMessage(taskId, 2, gText_BootedUpTM, BootUpSoundTMHM); // TM
}
...
static void Task_ShowTMHMContainedMessage(u8 taskId)
{
    if (JOY_NEW(A_BUTTON | B_BUTTON))
    {
        ...
        DisplayItemMessage(taskId, 2, gStringVar4, UseTMHMYesNo);
    }
}
...
static void Task_UseStatIncreaseItem(u8 taskId)
{
    if(++gTasks[taskId].data[8] > 7)
    {
        ...
        RemoveBagItem(gSpecialVar_ItemId, 1);
        if (!InBattlePyramid())
            DisplayItemMessage(taskId, 2, UseStatIncreaseItem(gSpecialVar_ItemId), Task_CloseStatIncreaseMessage);
        else
...
void ItemUseOutOfBattle_PokeDoll(u8 taskId)
{
    bool8  pokeDollOn = FlagGet(FLAG_POKE_DOLL);
    if (!pokeDollOn)
    {
