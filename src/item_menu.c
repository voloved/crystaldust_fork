...
static void MergeSort(struct ItemSlot* array, u32 low, u32 high, u8 pocket, s8 (*comparator)(struct ItemSlot*, struct ItemSlot*, u8));
static void Merge(struct ItemSlot* array, u32 low, u32 mid, u32 high, u8 pocket, s8 (*comparator)(struct ItemSlot*, struct ItemSlot*, u8));
static s8 CompareItemsAlphabetically(struct ItemSlot* itemSlot1, struct ItemSlot* itemSlot2, u8 pocket);
static s8 CompareItemsByMost(struct ItemSlot* itemSlot1, struct ItemSlot* itemSlot2, u8 pocket);
static s8 CompareItemsByType(struct ItemSlot* itemSlot1, struct ItemSlot* itemSlot2, u8 pocket);
static s8 CompareItemsByID(struct ItemSlot* itemSlot1, struct ItemSlot* itemSlot2, u8 pocket);
...
static const u8 sBlit_SelectButtonHold[] = INCBIN_U8("graphics/item_menu/select_button_hold.4bpp");
...
void BagMenu_ItemPrintCallback(u8 windowId, u16 index, s32 itemIndex, u8 y)
{
    ...
    if (itemIndex != LIST_CANCEL && gBagMenu->numItemStacks[gBagPosition.pocket] != itemIndex)
    {
        ...
        // Print registered icon
            ...
            BlitBitmapToWindow(windowId, sBlit_SelectButtonHold, 112, y, 24, 16);
    }
}
...
static void ItemMenu_Register(u8 taskId)
{
    ...
    registerSlot = tIsRegisterHold ? &gSaveBlock1Ptr->registerHoldItem : &gSaveBlock1Ptr->registeredItem;
...
static void Task_LoadRegisterOptions(u8 taskId)
{
    ...
    else if (gSaveBlock1Ptr->registerHoldItem == gSpecialVar_ItemId){
        // If the item is already registered, then deselect it.
...
{
    u8 taskId;
    u16 item = isRegisterHold ? gSaveBlock1Ptr->registerHoldItem : gSaveBlock1Ptr->registeredItem;
    ...
    if (isRegisterHold)
        ScriptContext1_SetupScript(EventScript_SelectWithoutRegisteredHoldItem);
    else
...
enum ItemSortBalls
{
	...
	ITEM_TYPE_THIEF_BALL,
	ITEM_TYPE_SAFARI_BALL,
    ITEM_TYPE_LEVEL_BALL,
    ITEM_TYPE_LURE_BALL,
    ITEM_TYPE_MOON_BALL,
    ITEM_TYPE_FRIEND_BALL,
    ITEM_TYPE_FAST_BALL,
    ITEM_TYPE_HEAVY_BALL,
    ITEM_TYPE_LOVE_BALL,
    ITEM_TYPE_PARK_BALL,
...
static const u16 sItemsByType[ITEMS_COUNT] =
{
    ...
    [ITEM_RAGE_CANDY_BAR] = ITEM_TYPE_HEALTH_RECOVERY,
    ...
    [ITEM_NEVER_MELT_ICE] = ITEM_TYPE_EVOLUTION_ITEM,
    ...
    [ITEM_BLU_APRICORN] = ITEM_TYPE_APRICORN,
    [ITEM_YLW_APRICORN] = ITEM_TYPE_APRICORN,
    [ITEM_GRN_APRICORN] = ITEM_TYPE_APRICORN,
    [ITEM_WHT_APRICORN] = ITEM_TYPE_APRICORN,
    [ITEM_BLK_APRICORN] = ITEM_TYPE_APRICORN,
    [ITEM_PNK_APRICORN] = ITEM_TYPE_APRICORN,
    ...
    [ITEM_SLOWPOKE_TAIL] = ITEM_TYPE_SELLABLE,
	...
	[ITEM_THIEF_BALL] = ITEM_TYPE_THIEF_BALL,
	[ITEM_SAFARI_BALL] = ITEM_TYPE_SAFARI_BALL,
    [ITEM_LEVEL_BALL] = ITEM_TYPE_LEVEL_BALL,
    [ITEM_LURE_BALL] = ITEM_TYPE_LURE_BALL,
    [ITEM_MOON_BALL] = ITEM_TYPE_MOON_BALL,
    [ITEM_FRIEND_BALL] = ITEM_TYPE_FRIEND_BALL,
    [ITEM_FAST_BALL] = ITEM_TYPE_FAST_BALL,
    [ITEM_HEAVY_BALL] = ITEM_TYPE_HEAVY_BALL,
    [ITEM_LOVE_BALL] = ITEM_TYPE_LOVE_BALL,
    [ITEM_PARK_BALL] = ITEM_TYPE_PARK_BALL,
};
...
static void SortBagItems(u8 taskId)
{
    ...
    BagMenu_RemoveWindow(ITEMWIN_LIST);
    BagMenu_RemoveWindow(ITEMWIN_6);
    //BagMenu_RemoveWindow(ITEMWIN_YESNO_LOW);
...
static void SortItemsInBag(u8 pocket, u8 type)
{
    ...
    case SORT_ALPHABETICALLY:
        MergeSort(itemMem, 0, itemAmount - 1, pocket, CompareItemsAlphabetically);
        break;
    case SORT_BY_AMOUNT:
        MergeSort(itemMem, 0, itemAmount - 1, pocket, CompareItemsByMost);
        break;
    case SORT_BY_NUMBER:
        MergeSort(itemMem, 0, itemAmount - 1, pocket, CompareItemsByID);
        break;
    default:
        MergeSort(itemMem, 0, itemAmount - 1, pocket, CompareItemsByType);
        break;
    }
}
...
static void MergeSort(struct ItemSlot* array, u32 low, u32 high, u8 pocket, s8 (*comparator)(struct ItemSlot*, struct ItemSlot*, u8))
{
    ...
    mid = low + (high - low) / 2;
    MergeSort(array, low, mid, pocket, comparator); //Sort left half.
    MergeSort(array, mid + 1, high, pocket,comparator); //Sort right half.
    Merge(array, low, mid, high, pocket,comparator); //Merge results.
}
...
static void Merge(struct ItemSlot* array, u32 low, u32 mid, u32 high, u8 pocket, s8 (*comparator)(struct ItemSlot*, struct ItemSlot*, u8))
{
    ...
    { //Merge back to a[low..high]
        if (i > mid)
            ...
            array[k] = aux[i++];
        else if (comparator(&aux[j], &aux[i], pocket) < 0)
            array[k] = aux[j++];
        else
            array[k] = aux[i++];
    }
...
static s8 CompareItemsByMost(struct ItemSlot* itemSlot1, struct ItemSlot* itemSlot2, u8 pocket)
{
    ...
    if (quantity1 < quantity2)
        ...
        return -1;
    if (pocket == TMHM_POCKET)
        return CompareItemsByType(itemSlot1, itemSlot2, pocket);
...
static s8 CompareItemsByType(struct ItemSlot* itemSlot1, struct ItemSlot* itemSlot2, u8 pocket)
{
    ...
    if (type1 < type2)
        ...
        return 1;
    return CompareItemsAlphabetically(itemSlot1, itemSlot2, pocket); //Items are of same type so sort alphabetically
}
